...
*/
int
iot_qos_set_limit_time(struct timeval *last_time, struct timeval *now, double gap){
...
iot_qos_set_next_limit(APP_Qos_Info *app, call_stub_t *stub){
	...
	double gap = iot_qos_get_gap(app->limit_bandwidth, app->block_size / 1024);
	
	//gf_log(stub->frame->this->name, GF_LOG_ERROR, "jy_message_set_next_timeval: gap: %f us", gap);
	return iot_qos_set_limit_time(&app->last_time, &stub->limit_time, gap);
}
...
iot_qos_set_next_reserve(APP_Qos_Info *app, call_stub_t *stub){
	...
	double gap = iot_qos_get_gap(app->reserve_bandwidth, app->block_size / 1024);

	ret = iot_qos_set_reserve_time(&app->last_reserve_time, &stub->reserve_time, gap);

	if(ret == _gf_false)
		app->last_time = stub->limit_time;
...
iot_qos_app_info_insert(iot_conf_t *conf, double bandwidth, double limit, const char* uuid, int IS_IOPS, double block_size){
	...
	new_app->reserve_bandwidth = bandwidth * 1024;
	if(conf->app_count == 0){
		new_app->weight = 1;
		new_app->limit_bandwidth = conf->default_BW;
		...
		double total_weight = bandwidth;
		{
			...
			list_for_each_entry (tmp_app, &conf->apps, apps){
				...
				tmp_app->weight = tmp_app->exp_bandwidth / total_weight;
				tmp_app->limit_bandwidth = tmp_app->weight * conf->default_BW;
				if(tmp_app->limit_bandwidth <= tmp_app->reserve_bandwidth){
					tmp_app->reserve_bandwidth = tmp_app->limit_bandwidth / 2.0;
					gf_log(conf->this->name, GF_LOG_ERROR, "jy_message_app_insert: app:%s limit < reserve!!!!!", tmp_app->uuid);
				}
		...
		new_app->weight = new_app->exp_bandwidth / total_weight;
		new_app->limit_bandwidth = new_app->weight * conf->default_BW;
		if(new_app->limit_bandwidth <= new_app->reserve_bandwidth){
			new_app->reserve_bandwidth = new_app->limit_bandwidth / 2.0;
			gf_log(conf->this->name, GF_LOG_ERROR, "jy_message_app_insert: app:%s limit < reserve!!!!!", new_app->uuid);
		}
	}
	...
	gf_log(conf->this->name, GF_LOG_ERROR, "jy_message_app_insert: insert app:%s, app_count:%d, app_reserve_bandwidth:%f MB/s, app_limit_bandwidth:%f MB/s", new_app->uuid, conf->app_count, new_app->exp_bandwidth, new_app->limit_bandwidth  / 1024);
...
iot_qos_app_reweight(iot_conf_t * conf){
	...
	list_for_each_entry (tmp_app, &conf->apps, apps){
		if(tmp_app->state == APP_SLEEP)
					break;
		pthread_mutex_lock(&tmp_app->mutex);
		tmp_app->reserve_bandwidth = tmp_app->exp_bandwidth * 1024;
		tmp_app->weight = tmp_app->exp_bandwidth / total_weight;
		tmp_app->limit_bandwidth = tmp_app->weight * conf->default_BW;
		if(tmp_app->limit_bandwidth <= tmp_app->reserve_bandwidth){
			tmp_app->reserve_bandwidth = tmp_app->limit_bandwidth / 2.0;
			gf_log(conf->this->name, GF_LOG_ERROR, "jy_message_app_reweight: app:%s limit < reserve!!!!!", tmp_app->uuid);
		}
		gettimeofday(&tmp_app->last_time,NULL);
		pthread_mutex_unlock(&tmp_app->mutex);
	
		gf_log(conf->this->name, GF_LOG_ERROR, "jy_message_app_reweight: reweight app:%s limit_bandwidth to %f MB/s", tmp_app->uuid, tmp_app->limit_bandwidth / 1024);
	}
...
set_app_bw(iot_conf_t * conf, const char* const msg, double v_wbw, double v_rbw, double v_diops){
	...
	//no support for rbw
	if(v_wbw != -1){
		app->exp_bandwidth = v_wbw;
	}
	else if(v_diops != -1){
		if(app->IS_IOPS != 1)
			app->IS_IOPS = 1;
		app->exp_bandwidth = v_diops * (app->block_size / (1024 * 1024));	//IOPS * block_size = B/s
	}
