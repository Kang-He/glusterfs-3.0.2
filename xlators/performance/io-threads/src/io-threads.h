...
typedef struct iot_qos_limit_worker limit_worker;
...
struct iot_conf {
        ...
        pthread_attr_t       w_attr;        /* Used to reduce the stack size of
                                            ...
                                            */
        //Qos test
        limit_worker* lworker;
        struct list_head apps;
		struct list_head priority_req;
		struct list_head normal_req;
		pthread_cond_t ot_notifier;
		pthread_cond_t limit_notifier;
        int app_count;
		int rand;
		struct timeval ctime;		//used for check app eviction and print app throughput
		void* sub;
		//default setting
		int default_reserve;
		int default_limit;
		int default_BW;	//Input Maximum bandwidth MB / s, all converted to KB / s
		char *redis_host;
		int redis_port;
